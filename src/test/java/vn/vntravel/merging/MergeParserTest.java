...
public class MergeParserTest {
    private List<MergeTopicModel> mergeTopicModels;
    ...
    private List<MergeTopicModel> runParseTest(String filename){
        return new MergeParser().parse(filename);
    }

    @Test
    public void TestParserModel() throws Exception{
        mergeTopicModels = runParseTest("src/test/resources/merge-test.properties");
        assertEquals("Check number object: ",2, mergeTopicModels.size());
        assertEquals("Check number topics object1: ", 3, mergeTopicModels.get(0).getTopics().size());
        assertEquals("Check number topics object2: ",2, mergeTopicModels.get(1).getTopics().size());
    }
    ...
    public void TestFindColumnJoin() throws Exception{
        ...
        MergeTopicModel model1 = mergeTopicModels.get(0);
        assertEquals("Check size column-join object1: ",2, model1.getMergerMaps().size());
        ...
        MergeTopicModel model2 = mergeTopicModels.get(1);
    ...
    public void testWithErrorFormFile() throws Exception{
        mergeTopicModels = runParseTest("src/test/resources/wrong-format-test.properties");
        assertEquals("Check right-format object size: ", 1, mergeTopicModels.size());
    }
}
